...
|--------------------------------------------------------------------------
|
| Every route behind the sign-in screen. Registered by bootstrap/app.php inside
| a group carrying config('app.admin_prefix'), so setting ADMIN_ROUTE_PREFIX in
| .env moves the whole back office under /admin, /backoffice or whatever it says
| — and a route added here cannot forget to move with it.
|
| The public landing page is NOT here. It stays at `/` in routes/web.php, because
| it is the one address a customer is given.
|
| Route names are untouched by the prefix, so every route() call and every test
