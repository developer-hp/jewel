...
@section('title', 'Item Estimates')

@include('layouts.partials.datatables-assets')

@section('content')
    <x-page-title title="Item Estimates">
        <x-slot:actions>
            @can('item_estimate.create')
                <a href="{{ route('item-estimates.create') }}" class="btn btn-primary">
    ...
    @can('item_estimate.print')
        <form method="POST" action="{{ route('item-estimates.print') }}" target="_blank" id="print-form">
            @csrf
    ...
    <script>
        $(function () {
            ...
            const table = window.appDataTable('#estimates-table', {
                ajax: '{{ route('item-estimates.index') }}',
                order: [[1, 'desc']],
                columns: [
                    ...
                    { data: 'og_ref', name: 'og_ref', orderable: false, searchable: false },
                    { data: 'net', name: 'net', orderable: false, searchable: false, className: 'text-end' },
