...
@section('content')
    ...
    {{-- One flowing grid rather than a tall column beside a short one: the
         details card used to end halfway down the page and leave the rest of
         its column blank. Paired cards carry h-100 so they match height. --}}
    <div class="row g-3">
        <div class="col-lg-8">
        ...
        {{-- Side by side when the piece has both, full width when only one. --}}
        @php($kinds = collect([['stone', 'Stones'], ['diamond', 'Diamonds']])
            ->filter(fn ($pair) => $item->itemStones->where('kind', $pair[0])->isNotEmpty())
            ->values())

        @foreach ($kinds as [$kind, $label])
            @php($rows = $item->itemStones->where('kind', $kind))
            <div class="col-lg-{{ $kinds->count() > 1 ? 6 : 12 }}">
                <div class="card h-100 mb-0">
                    <div class="card-header py-2">
