{{--
    The sticker that goes on the bag.
    ...
    row will not hold a page break where it is told to. Kept in step with
    repair-forms/stickers, which is the same job on the same paper.

    The Ready column is left blank on purpose — the bench ticks it by hand as each
    piece is finished. What the system considers ready is read off stock, on the
    order listing.
--}}
@extends('layouts.pdf')
...
@section('content')
    @foreach ($forms->chunk($perSheet) as $sheet)
